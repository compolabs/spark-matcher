import { arrayify, CoinQuantityLike, hashMessage, WalletLocked, WalletUnlocked } from "fuels";
...
import { CONTRACT_ADDRESSES, GAS_LIMIT, TOKENS_BY_ASSET_ID, TOKENS_BY_SYMBOL } from "./constants";
...
export class Api {
  ...
  matchSpotOrders = async (
    ...
    const tx = orderbookFactory.functions
      .match_orders(sellOrder, buyOrder)
      .txParams({ gasPrice: 1 });
  ...
  createSpotOrder = async (
    ...
    const amountToSend = new BN(absSize)
      .times(price)
      .dividedToIntegerBy(
        new BN(10).pow(DEFAULT_DECIMALS + baseToken.decimals - quoteToken.decimals)
      );
  ...
  const {gasUsed: gasValue} = await account.contracts?.vaultContract.functions.deposit_collateral()
              .callParams({
          ...
          const freeCollBefore = await account.contracts?.vaultContract.functions.deposit_collateral()
              .callParams({
                  forward: {amount: amount.toString(), assetId: TOKENS_BY_SYMBOL.USDC.assetId},
              })
              .txParams({gasPrice: 1, gasLimit: gasValue})
              .call();
    * */
  ...
  mintToken = async (
    ...
    const amount =
      mintAmount ?? BN.parseUnits(FAUCET_AMOUNTS[token.symbol].toString(), token.decimals);
    ...
    const { gasUsed: gasValue } = await tx.getTransactionCost();
    await tx.txParams({ gasLimit: gasValue }).call();
  };
  ...
  openPerpOrder = async (
    ...
    const market = await clearingHouseFactory.functions.get_market(assetIdInput).simulate();

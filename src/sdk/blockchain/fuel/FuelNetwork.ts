...
export class FuelNetwork extends BlockchainNetwork {
  ...
  createSpotOrder = async (assetAddress: string, size: string, price: string): Promise<string> => {
    ...
    return this.api
      .createSpotOrder(baseToken, quoteToken, size, price, this.walletManager.wallet)
      .then((res) => res.txId);
  };

  createSpotMarket = async (baseTokenAddress: string, decimals: number): Promise<string> => {
    if (!this.walletManager.wallet) {
      throw new Error("Wallet does not exist");
    }
    ...
    return this.api.createSpotMarket(baseToken, decimals, this.walletManager.wallet);
  };

  matchSpotOrders = async (sellOrder: string, buyOrder: string): Promise<string> => {
    if (!this.walletManager.wallet) {
      throw new Error("Wallet does not exist");
    }
